...
\end{equation} 
on les mostres ${v^{(t)}}$ s s'assumeixen que són independents entre si, i els estats h de les neurones ocultes han de ser marginades.\\\\
...
S'han proposat molts enfocaments per superar aquestes dificultats. Un enfocament popular és l'aprenentatge de divergència contrastiva (CD) com un mètode aproximat per a la formació dels models Product-of-Expert. L'equació (4.8) implica directament que una RBM és un cas especial de models PoE, i l'aprenentatge de CD pot ser utilitzat fàcilment per formar una RBM.\\\\
L'aprenentatge de CD s'aproxima al veritable gradient reemplaçant l'expectativa sobre ${P(v,h | \theta)}$ amb una expectativa sobre una distribució ${P_n}$ que s'obté executant ${n}$ passos del mostreig de Gibbs a partir de la distribució empírica definida per les mostres d'entrenament. La figura 11 il·lustra les distribucions ${P_0}$ i ${P_n}$.
\begin{figure}[h!]
\begin{center}
...
Com es pot preveure del fet que la direcció del gradient no és idèntica al gradient exacte, se sap que l'aprenentatge de CD està parcial \cite{carreira}. No obstant això, s'ha demostrat que l'aprenentatge en CD funciona bé a la pràctica. Una bona propietat del CD és que, en el cas que la distribució de dades sigui multimodal, les cadenes a partir de cada mostra de dades garanteix que les mostres que s'aproximen a la fase negativa tenen representants de diferents maneres.\\\\
...
\end{figure}
\\\\
Per superar aquest problema, s'han proposat diferents enfocaments basats en l'aprenentatge de CD. Entre ells, l'aprenentatge persistent de divergència contrastiva (PCD) és l'extensió més senzilla de l'aprenentatge de CD.\\\\
...
S'han proposat recentment nombrosos enfocaments basats en l'aprenentatge de CDs diferents de l'aprenentatge de la PCD. Per exemple, s'ha proposat un aprenentatge que amplia l'aprenentatge de la PCD mantenint uns pesos ràpids que ajuden a obtenir millors models de mostres.\\\\
