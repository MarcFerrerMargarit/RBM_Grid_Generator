...
També vull agrair al meu tutor a l'empresa, David Osuna, per tal de proposar-me aquest projecte tant important i tant interessant per a realitzar i per ajudar-me en tots els dubtes que he tingut durant aquest temps.\\\
I finalment vull agrair al meu tutor del treballa, David Buchaca, per tal d'haver-me ajudat a trobar a dur a terme el projecte correctament i per haver-me ajudat a trobar eines i solucions per tal de resoldre els problemes que se'm presentaven al llarg del desenvolupament.
\newpage
...
Per motius de confidencialitat la empresa no ha deixat publicar ni adjuntar el codi en aquesta memoria. De totes maneres, s'han inclos parts de codi on es pot veure com s'han entrenat els models i els preprocessos necessaris per a generar els inputs.
...
dada de la eina -> ALGORITHM 3  -> ALGORITHM 4 -> OneHOtencoder -> RBM -> RBM.gibbs -> Algorithm5 -> Dada de sortida (nova graella)
...
Definirem tres tipus d'avaluació diferents per tal d'avaluar el nostre sistema creat. La primera avaluació serà una avaluació qualitativa. Es generaran 10 moodboars utilitzant la màquina i al eina respectivament i se'ls ensenyaran al expert i haurà de valorar per cada un d'ells quins són els objectes que fallen i perquè. És important enfatitzar que degut a dificultats tecniques sols disposavem de 8 dades d'entrenament.
...
Això pot ser degut a com es realitza el filtratge o com està implementada la base de dades, ja que en cap moment podem filtrar per estil que nosaltres volem ja que a dins la base de dades aquest paràmetre és inaccessible. Això provoca que la llista d'objectes que ens retorna sigui més gran i per tant contingui objectes amb estils diferents als que desitgem però que compleixen les altres propietats. Així doncs, la solució seria poder filtrar segons l'estil corresponen a la imatge per tal que els objectes que ens retornen concordin amb l'estil seleccionat.\\\\
L'altre raó per la qual molts objectes s'han marcat malament és pels colors que el formen. La màquina no té en compte la relació de colors entre objectes, no estableix cap relació entre els objectes que es col·loquen ja que només s'analitzen les característiques d'aquests. Així doncs és normal que els colors dels objectes no tinguin relació quan en la generació prèvia de moodboards l'expert col·locava els objectes per tal de tenir coherència entre ells però alhora d'entrenar la màquina aquesta relació no s'analitza profundament.\\\
Així doncs, és normal, que surtin objectes amb colors que no tenen relació tot i que el filtratge amb la base de dades hagi donat com a vàlids aquests objectes.
\subsubsection{Possibles problemes}

\begin{itemize}
\item El nostre dataset de entrenament tenia tan sols 8 graelles.
\item El filtratge de les dades de sortida és incorrecte.
\end{itemize}
