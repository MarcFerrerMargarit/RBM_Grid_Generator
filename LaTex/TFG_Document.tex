...
La probabilitat d'un estat particular ${X = [x_1, x_2,...,x_d]^T}$ de la xarxa es defineix per l'energia de BM que es postula com \[E (x | ?) = - X i X j> i wijxixj - X i Bixi\] on ${?}$ indica paràmetres de la xarxa que consisteixen en una matriu de pes ${W = [wij]}$ i un vector de polarització ${b = [bi]}$. wij és el pes de les connexions sinàptiques entre les neurones i i j. Suposem que wii = 0 i wij = wji. La probabilitat d'un estat x és, doncs, \[P (x | ?) = 1 Z (?) exp [-E (x | ?)] (2.1) \] on \[Z (?) = X x exp [-E (x | ?)]\] és la constant de normalització. Es dedueix de (2.1) que la probabilitat condicional d'una sola neurona que sigui 0 o 1 donada als estats de les altres neurones es pot escriure de la següent manera: \[P (xi = 1 | x \ i, W) = 1 1 + exp - P j6 = i wijxj - bi, (2.2)\] on x \ denota un vector [x1, · · ·, xi-1, xi + 1, · · ·, xd] T. \\\\
