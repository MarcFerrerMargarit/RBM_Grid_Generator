...
\item ${fit_transform (X[,y])}$: ens permet entrenar la màquina a partir de les dades X i després transformar-la.
\item ${get_params([deep])}$: obtenim els paràmetres del model que hem generat.
\item ${gibbs(v)}$: s'executa un mostreig de Gibbs.
\item ${partial_fit(X[,y])}$: ens permet entrenar la màquina a partir de les dades X que han de contenir una part segmentada de les dades.
\item ${score_samples(X)}$: computació de la probabilitat de X.
\item ${set_params(**params)}$: estableix els paràmetres del model.
\item ${transform(X)}$: computació de les probabilitats d'activació de la capa oculta.
\end{itemize}
Un exemple del ús d'aquesta màquina es troba en l'obtenció de features per la classificació de dígits.\\\\
Un cop tenim la implementació que usarem ara només ens queda preparar les dades per la entrada a la màquina.
\newpage
